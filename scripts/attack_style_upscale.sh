# ------------------------- Train ASPL on set B -------------------------
export EXPERIMENT_NAME="vangogh_with_styleloss_SD14_upscaling"
...
# accelerate launch --num_processes=4 --mixed_precision="fp16" --gpu_ids="0,1,2,3" --config_file gpu_config.yaml  --main_process_port=8889 attacks/time_feature_style.py \
...
export INSTANCE_DIR="outputs/style/wikiart/vangogh_with_styleloss_SD14/noise-upscaling"

python Noise_Upscaling.py --input_folder="outputs/style/wikiart/vangogh_with_styleloss_SD14/noise-ckpt/50" \
  --output_folder=$INSTANCE_DIR 

accelerate launch --num_processes=4 --gpu_ids="0,1,2,3" --config_file gpu_config.yaml --main_process_port=8830 /home/yjli/AIGC/diffusers/examples/dreambooth/train_dreambooth.py \
  --pretrained_model_name_or_path=$MODEL_PATH  \
