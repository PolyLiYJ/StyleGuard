...
export EXPERIMENT_NAME="finegrain-image-enhanser"
...
export INSTANCE_DIR="$OUTPUT_DIR/upscaled_image_512"
...
# accelerate launch train_dreambooth.py \
accelerate launch --num_processes=2 --gpu_ids="6,7" --config_file gpu_config.yaml --main_process_port=8888 /home/yjli/AIGC/diffusers/examples/dreambooth/train_dreambooth.py \
  --pretrained_model_name_or_path=$MODEL_PATH  \
  --enable_xformers_memory_efficient_attention \
  --train_text_encoder \
  --instance_data_dir=$INSTANCE_DIR \
...
python infer.py \
  --model_path $DREAMBOOTH_OUTPUT_DIR \
  --output_dir $DREAMBOOTH_OUTPUT_DIR/checkpoint-1000-test-infer \
  --prompt "an sks painting, including a people walking on the street"
